...
const RSI_LABELS = Array.from({ length: 25 }, (_, i) => (i === 24 ? "W0" : `W-${24 - i}`));
...
const consistencyStatus=(c)=>c>=18?"Highly Consistent":c>=12?"Moderately Consistent":c>=6?"Weak Consistency":"Low Consistency";
...
function getDivergence(priceChange12W, rsiChange12W, rsiChange4W) {
  ...
  if (priceChange12W > 3 && rsiChange12W > 0) { status = "Price and RSI Aligned"; note = "Price and weekly momentum are moving in the same direction."; if (rsiChange4W < 0) note = "Long-range alignment remains positive, but short-term RSI is cooling."; }
  else if (priceChange12W > 3 && rsiChange12W < 0) { status = "Bearish Divergence"; note = "Price improved while weekly RSI weakened, showing softer momentum confirmation."; if (rsiChange4W < 0) note = "Momentum divergence is also visible in the short-term window."; }
  else if (priceChange12W < -3 && rsiChange12W > 0) { status = "Bullish Divergence"; note = "Price weakened while weekly RSI improved, showing momentum recovery."; if (rsiChange4W > 0) note = "Weekly RSI recovery is also improving in the short-term window."; }
  else if (priceChange12W >= -3 && priceChange12W <= 3 && rsiChange12W > 0) { status = "Momentum Improving During Consolidation"; note = "Price is consolidating while weekly RSI improves."; }
  else if (priceChange12W >= -3 && priceChange12W <= 3 && rsiChange12W < 0) { status = "Momentum Cooling During Consolidation"; note = "Price is consolidating while weekly RSI cools."; }
  return { status, note };
}
...
function runBiasScanner(prices, rsis){
  ...
  for(let i=0;i<prices.length;i++){
    for(let j=i+3;j<prices.length && j-i<=24;j++){
      const priceChangePercent=((prices[j]-prices[i])/prices[i])*100;
      const rsiChange=rsis[j]-rsis[i];
      if(Math.abs(priceChangePercent)<3 && Math.abs(rsiChange)<4) continue;
      let bias="";
      if((priceChangePercent<=-3 || (priceChangePercent>=-3&&priceChangePercent<=3)) && rsiChange>=4) bias="Potential Upward Bias";
      else if((priceChangePercent>=3 || (priceChangePercent>=-3&&priceChangePercent<=3)) && rsiChange<=-4) bias="Potential Downward Bias";
      if(!bias) continue;
      const score=Math.abs(priceChangePercent)+Math.abs(rsiChange);
      out.push({bias, range:`W-${24-i} → W-${24-j}`, priceChangePercent, rsiChange, score, confidence:getConfidence(score)});
    }
  }
...
function setLoading(){
  ...
  els.rsiChangeLines.textContent="4W: — · 12W: — · 24W: —"; els.rsiRefs.textContent="W0: — | W-4: — | W-12: — | W-24: —"; els.rsiSlope.textContent="RSI Slope 24W: — / week";
  els.direction.textContent="—"; els.momentumPhase.textContent="—";
  els.analyticsStrip.textContent="RSI Slope 12W: — · RSI Slope 24W: — · Consistency 12W: — · Consistency 24W: — · Distance to 50: — · Regime: —";
  els.priceMetrics.textContent="12W Price Change: — | 12W RSI Change: — | 24W Price: — | 24W RSI: —";
...
function updateRsiAndPrice(klines){
  ...
  const latestPrices=closes.slice(-RSI_WINDOW);
  const rsiSeries=calculateRsiSeries(closes);
  if(rsiSeries.length<RSI_WINDOW||latestPrices.length<RSI_WINDOW) throw new Error("Not enough weekly data");

  const v=rsiSeries.slice(-RSI_WINDOW).map(n=>Number(n.toFixed(1)));
  const w0=v[24], w1=v[23], w4=v[20], w12=v[12], w24=v[0];
  const d4=w0-w4, d12=w0-w12, d24=w0-w24;
  const slope12=d12/12, slope24=d24/24, distance50=w0-50;
  const direction=getDirection(w0,w1,w4,w12), phase=getPhase(w0,d4,d12,w4,w12), regime=getRegime(w0);
  let rises12=0; for(let i=13;i<v.length;i++) if(v[i]>v[i-1]) rises12+=1;
  let rises24=0; for(let i=1;i<v.length;i++) if(v[i]>v[i-1]) rises24+=1;

  const p0=latestPrices[24], p4=latestPrices[20], p12=latestPrices[12], p24=latestPrices[0];
  const priceChange4W=((p0-p4)/p4)*100, priceChange12W=((p0-p12)/p12)*100, priceChange24W=((p0-p24)/p24)*100;
  const divergence=getDivergence(priceChange12W,d12,d4);

  els.rsiCurrent.textContent=f1(w0);
  els.rsiStatus.textContent=classifyRsi(w0);
  els.rsiRegime.textContent=`Regime: ${regime}`;
  els.rsiChangeLines.textContent=`4W: ${signed1(d4)} · 12W: ${signed1(d12)} · 24W: ${signed1(d24)}`;
  els.rsiRefs.textContent=`W0: ${f1(w0)} | W-4: ${f1(w4)} | W-12: ${f1(w12)} | W-24: ${f1(w24)}`;
  els.rsiSlope.textContent=`RSI Slope 24W: ${signed2(slope24)} / week`;
  els.direction.textContent=direction;
  els.momentumPhase.textContent=phase;
  els.analyticsStrip.textContent=`RSI Slope 12W: ${signed2(slope12)}/week · RSI Slope 24W: ${signed2(slope24)}/week · Consistency 12W: ${rises12}/12 · Consistency 24W: ${rises24}/24 · Distance to 50: ${signed1(distance50)} (${midlineStatus(distance50)}) · Regime: ${regime}`;

  els.priceMetrics.textContent=`12W Price Change: ${signed1(priceChange12W)}% | 12W RSI Change: ${signed1(d12)} | 24W Price: ${signed1(priceChange24W)}% | 24W RSI: ${signed1(d24)}`;
  els.divergenceStatus.textContent=`Divergence Status: ${divergence.status}`;
  els.divergenceText.textContent=`4W context: Price ${signed1(priceChange4W)}%, RSI ${signed1(d4)}. ${divergence.note}`;

  renderRsiChart(v);
  renderPriceChart(latestPrices);
  renderBiasScanner(runBiasScanner(latestPrices, v));
}
